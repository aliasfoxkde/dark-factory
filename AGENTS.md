...
This is **Dark Factory** — an opinionated GitHub repository template for bootstrapping production-grade software projects with 90%+ automated test, code, and documentation coverage.
...
- Run `make lint` and `make test` before requesting review
...
- Use `template-parts/` to assemble project-specific templates
- Replace `{{GITHUB_OWNER}}` and `{{PROJECT_NAME}}` placeholders
- Run `make setup` to install git hooks
- Review `docs/` for project-specific documentation
